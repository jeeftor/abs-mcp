...
| `abs_update_item_metadata` | Planned item metadata update. | `itemId` required; `payload` reserved until safe typed fields are source-verified. | Not implemented yet. | Planned mutation; advertised for discovery; blocked by read-only mode. | `ABS_READ_ONLY=true`; missing `itemId`; not implemented until safe typed metadata fields are verified. |
